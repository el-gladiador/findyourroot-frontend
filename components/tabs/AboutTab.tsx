...
const AboutTab = () => {
  const socialLinks = [
    { icon: Github, url: `https://github.com/${CREATOR_INFO.socials.github}`, available: !!CREATOR_INFO.socials.github },
    { icon: Instagram, url: `https://instagram.com/${CREATOR_INFO.socials.instagram}`, available: !!CREATOR_INFO.socials.instagram },
    { icon: Linkedin, url: `https://linkedin.com/in/${CREATOR_INFO.socials.linkedin}`, available: !!CREATOR_INFO.socials.linkedin },
  ];
      ...
      <div className="flex flex-col items-center mb-8">
        <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600 p-1 mb-4 shadow-xl shadow-indigo-500/30">
          <div className="w-full h-full rounded-full bg-white dark:bg-slate-900 flex items-center justify-center overflow-hidden">
             <img src="/me.jpg" alt="Zaki Amiri" className="w-full h-full object-cover" />
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-8">
          {socialLinks.map(({ icon: Icon, url, available }, i) => (
            available ? (
              <a 
                key={i} 
                href={url}
                ...
                className="p-3 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-600 dark:text-slate-400 hover:bg-indigo-100 hover:text-indigo-600 dark:hover:bg-slate-700 transition-all"
              >
                <Icon size={20} />
              </a>
            ) : (
              <span 
                key={i} 
                className="p-3 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-300 dark:text-slate-600 cursor-not-allowed opacity-50"
              >
                <Icon size={20} />
              </span>
            )
